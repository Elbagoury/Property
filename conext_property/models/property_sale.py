...
class property_sale(models.Model):
    ...
    def _compute_amount(self):
        ...
        bunga = (self.amount_after_discount) * (self.interest/100)
    ...
    pembayaran              = fields.One2many(comodel_name="payment.plan", inverse_name="order_id", string="Pembayaran")
    ...
    def compute_installment(self):
        payment_line = self.env['payment.plan']
    ...
    def onchange_amount_dp(self):
        ...
        if self.amount_dp and self.dp_method :
            if self.dp_method == 'percent':
                amount = (self.disc_pp_percent/100) * self.amount_before_discount
                amount_after = self.amount_before_discount - amount
                amount = (self.amount_dp/100) * self.amount_final
...
class property_order_line(models.Model):
    ...
    property_unit_id = fields.Many2one (comodel_name="product.template", string="Unit")
...
ooconfirm =[('draft','Draft'),('confirm','Confirm'),('paid','Paid'),('unpaid','Not Paid'),('cancel','Cancel')]
