...
class reserved(models.Model):
    ...
    property_unit_id = fields.Many2one(comodel_name="product.template", string="Unit Property")
    ...
    def button_confirmed(self):
        if self.name == '/':
            number = self.env['ir.sequence'].get('reserved_property')
            self.name = number
        prop_unit = self.env['product.template'].browse(self.property_unit_id.id)
